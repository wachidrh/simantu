...
class Peralatan extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['m_lokasi'] = $this->peralatan->get_all_m_lokasi();

		$data['javascript'] = 'setup/peralatan/peralatan.js';
		// var_dump($data['m_peralatan']);
		// exit;
		$this->template->load('template', 'setup/peralatan', $data);
	}
	...
	public function list()
	{
		...
		foreach ($list as $ls) {
			...
			$row[] = '<p href="#" class="text-primary text-hover-primary mb-1">' . $ls['nama_lokasi'] . '</p>';
	...
	public function store()
	{
		$input = array(
			...
			'id_m_lokasi' => $this->security->xss_clean($this->input->post('id_m_lokasi')),
